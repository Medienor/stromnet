...
interface TrondheimProductListProps {
  deals: any[];
  priceData: any;
...
const TrondheimProductList: React.FC<TrondheimProductListProps> = ({
  deals,
  priceData,
  annualConsumption,
  expandedDeal,
  ...
  if (!deals.length || !priceData) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600">Ingen strømavtaler tilgjengelig for Trondheim for øyeblikket.</p>
      </div>
    );
  }

  // Use the correct Trondheim price (NO3 area)
  const trondheimSpotPrice = priceData.trondheimAverage || priceData.nationalAverage || 69.42; // Fallback to a realistic value
  
  // Filter and sort deals for Trondheim
  const trondheimDeals = deals
    .map(deal => ({
      ...deal,
      // Recalculate with Trondheim prices
  ...
  return (
    <div className="bg-white shadow overflow-hidden rounded-lg">
      ...
      <div className="md:hidden">
        {trondheimDeals.map((deal, index) => {
          const isExpanded = expandedDeal === `trondheim-${deal.id}`;
          const priceBreakdown = getPriceBreakdown(deal, annualConsumption, trondheimSpotPrice);
          
          return (
            <div key={`trondheim-${deal.id}`} className={`p-4 border-b ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="bg-gray-50 p-2 rounded">
                  ...
                  <div className="font-medium">
                    {formatPrice(trondheimSpotPrice + deal.addonPrice)} øre
                  </div>
                </div>
                <div className="bg-gray-50 p-2 rounded">
              ...
              <div className="flex justify-between items-center">
                <button
                  onClick={() => toggleDealExpansion(`trondheim-${deal.id}`)}
                ...
                <div className="mt-4 pt-4 border-t border-gray-200">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-600">Nordpool spotpris Trondheim (NO3)</span>
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-600">Forbruk {formatConsumption(priceBreakdown.monthlyConsumption)} kWt</span>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {trondheimDeals.map((deal, index) => {
              ...
              return (
                <React.Fragment key={`trondheim-${deal.id}`}>
                          ...
                          <div>
                            ...
                            <dl className="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
                              ...
                              <div className="sm:col-span-1">
                                ...
                                <dd className="mt-1 text-sm text-gray-900">Trondheim (NO3)</dd>
                              </div>
                            </dl>
                          </div>
                        </div>
      ...
      <div className="bg-gray-50 px-6 py-4">
        <p className="text-sm text-gray-600">
          Den billigste strømavtalen i Trondheim per {new Date().toLocaleDateString('nb-NO', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
          })} er {trondheimDeals[0]?.name} fra {trondheimDeals[0]?.provider?.name}. 
...
export default TrondheimProductList; 