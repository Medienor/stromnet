...
interface BergenProductListProps {
  deals: any[];
  priceData: any;
...
const BergenProductList: React.FC<BergenProductListProps> = ({
  deals,
  priceData,
  annualConsumption,
  expandedDeal,
  ...
  // Use the correct Bergen price (NO5 area)
  const bergenSpotPrice = priceData.bergenAverage || priceData.nationalAverage || 71.84; // Fallback to a realistic value
  
  // Filter and sort deals for Bergen
  const bergenDeals = deals
    .map(deal => ({
      ...deal,
      // Recalculate with Bergen prices
    ...
    <div className="bg-white shadow overflow-hidden rounded-lg">
      ...
      <div className="md:hidden">
        {bergenDeals.map((deal, index) => {
          ...
          return (
            <div key={`bergen-${deal.id}`} className={`p-4 border-b ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
              ...
              {isExpanded && priceBreakdown && (
                <div className="mt-4 pt-4 border-t border-gray-200">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-600">Månedlig forbruk</span>
                      <span className="font-medium">{formatConsumption(priceBreakdown.monthlyConsumption)} kWt</span>
                    </div>
                    {priceBreakdown.monthlyFee > 0 && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Fast beløp per måned</span>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div>
                        ...
                        <span className="ml-2">{getProductTypeNorwegian(deal.productType)}</span>
                      </div>
                      {deal.agreementTime > 0 && (
                        <div>
                          <span className="text-gray-600">Bindingstid:</span>
                          <span className="ml-2">{deal.agreementTime} {deal.agreementTimeUnit === 'year' ? 'år' : 'måneder'}</span>
                        </div>
                      )}
                      <div>
                        <span className="text-gray-600">Prisområde:</span>
                        <span className="ml-2">Bergen (NO5)</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="bg-gray-50 px-6 py-4">
        <p className="text-sm text-gray-600">
          Den billigste strømavtalen i Bergen per {new Date().toLocaleDateString('nb-NO', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
...
export default BergenProductList; 